...
function createForceSimulation(width, height) {
    const frx = forceX(width / 2).strength(CONST.FORCE_X);
    const fry = forceY(height / 2).strength(CONST.FORCE_Y);

    const simulation = forceSimulation()
            .force('charge', forceManyBody().strength(CONST.FORCE_IDEAL_STRENGTH))
            .force('x', frx)
            .force('y', fry);
