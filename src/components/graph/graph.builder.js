...
function buildLinkProps(link, nodes, links, config, linkCallbacks, highlightedNode, highlightedLink, transform) {
    ...
    const x1 = ((nodes[source] && nodes[source].x) || 0) + config.offsetX;
    const y1 = ((nodes[source] && nodes[source].y) || 0) + config.offsetY;
    const x2 = ((nodes[target] && nodes[target].x) || 0) + config.offsetX;
    const y2 = ((nodes[target] && nodes[target].y) || 0) + config.offsetY;
...
function buildNodeProps(node, config, nodeCallbacks = {}, highlightedNode, highlightedLink, transform) {
    ...
    return {
        ...
        cx: (((node && node.x) || 0) + config.offsetX).toString(),
        cy: (((node && node.y) || 0) + config.offsetY).toString(),
        fill,
        fontColor,
        fontSize: fontSize * t,
        dx,
